...
async(conn, mek, m, { isAdmin, isGroup, reply, from }) => {
    ...
    const warnEntries = Object.entries(groupWarns).filter(([_, count]) => count > 0);
    ...
    for (const [user, count] of warnEntries) {
        warnList += `👤 @${user.split('@')[0]}: ${count} warning${count > 1 ? 's' : ''}\n`;
        totalWarns += count;
    }
    
    warnList += `\n📊 *Total:* ${totalWarns}`;
    warnList += `\n👥 *Users:* ${warnEntries.length}`;

    await reply(warnList, { mentions: warnEntries.map(([user]) => user) });
});

// ============ RESET ALL WARNS COMMAND ============
cmd({
...
async(conn, mek, m, { isAdmin, isGroup, reply, from }) => {
    ...
    const groupWarns = await getGroupWarns(from);
    const hasWarns = Object.values(groupWarns).some(count => count > 0);
...
module.exports = {
    on: 'body',
    function: async (conn, mek, m, { from, isGroup, sender, isBotAdmins, isAdmin }) => {
        try {
            if (!isGroup) return;
            if (isAdmin) return;
            ...
            const linkData = analyzeLinks(text);
            if (!linkData) return;
            ...
            if (action === 'warn') {
                userWarns = await addUserWarn(from, sender);
                if (userWarns >= warnCount) {
                    shouldKick = true;
                }
            }
            
            let reportMsg = formatLinkReport(linkData, sender, action, userWarns, warnCount);
            
            if (action === 'kick' || shouldKick) {
                try {
                    await conn.groupParticipantsUpdate(from, [sender], 'remove');
                    if (action === 'warn' && shouldKick) {
                        reportMsg = `👢 *Auto-Kicked!*\n@${sender.split('@')[0]} reached ${warnCount} warnings.`;
                    }
                } catch (e) {
                    console.error('Failed to kick:', e);
                    reportMsg += '\n\n❌ Failed to kick (need admin permissions)';
                }
            }
